...
while true; do
  read -p "Do you wish to edit the created fpkg file? [yn]: " yn
  case $yn in
  [Yy]*)
    echo "Starting up a temporary bash shell which you can use to make your edits"
    echo "Press Ctrl-D to finalize the package"
    echo "You are currently in the outdir of the package"
    cd "$out"
    bash
    cd "$old_pwd"
    break
